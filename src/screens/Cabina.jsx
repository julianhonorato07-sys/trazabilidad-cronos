...
const ACCIONES = {
  ...
  en_reparacion: [
    { nuevo: 'liberada', label: 'Liberar cabina ✓', css: 'success' },
...
export default function Cabina() {
  ...
  const todas = incidencias((i) => i.unidad && i.unidad.tipo === 'cabina')
  const abiertas = todas.filter((i) => !i.cerrada_at)

  const unidad = cis.length >= 3 ? unidadPorCis(cis, 'cabina') : null
  ...
  const validar = () => {
    if (cis.length < 3) return 'Ingresá un código de cabina válido (mínimo 3 caracteres).'
  ...
  return (
    <div>
      <h3>Control de Cabinas</h3>
      <p className="sub">Registrá desvíos específicos de cabinas y controlá su estado de reparación.</p>

      {/* Registrar Desvío */}
      <div className="card" style={{ marginBottom: 24, padding: 20 }}>
        <h4 style={{ margin: '0 0 12px' }}>Registrar nuevo desvío en Cabina</h4>
        {err && (
        ...
        {ok && (
          <div className="banner success" style={{ marginBottom: 12, backgroundColor: 'var(--green-soft)', color: 'var(--green)', border: '1px solid var(--green)', padding: '8px 12px', borderRadius: 8 }}>
            <span>✓ Cabina <strong>{ok}</strong> registrada con éxito.</span>
          </div>
        )}
        ...
        <div className="fila" style={{ gap: 12, marginBottom: 12 }}>
          <input
            ...
            placeholder="Código de Cabina (ej: CAB-01)"
            value={cis}
        ...
        {cis.length >= 3 && !unidad && (
          <div className="unidad-info nueva" style={{ marginBottom: 12 }}>
            ...
            <span>Nueva cabina. Opcional - Color identificador:</span>
        ...
        <h5 style={{ margin: '12px 0 6px', fontSize: 13, textTransform: 'uppercase', color: 'var(--faint)' }}>Fallas de Cabina</h5>
      ...
      <h4>Monitoreo de Cabinas en Piso ({abiertas.length})</h4>
      <div className="seg">
        {COLS.map((c) => (
          ...
          return (
            <section key={c.id} className={'col' + (c.id === tab ? ' on' : '')}>
              ...
              <div className="lista">
                ...
                {!lista.length && <Vacio texto="Sin cabinas en este estado" />}
              </div>
            </section>
          )
        })}
